...
def annealing_cost_function(coordinates_stations, current_station, step_station,
    ...
    x1, y1 = coordinates_stations[step_station]
    x2, y2 = coordinates_stations[destination]
...
def find_nearest_connection(coordinates_stations, current_station, needed_connections_dict):
    ...
    as an object.
    """
    x1, y1 = coordinates_stations[current_station]
    ...
    for connection in needed_connections_dict:
        x2, y2 = coordinates_stations[needed_connections_dict[connection].start_station]
        euclidean_distance = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
...
def trim_trajectory(trajectory, needed_connections_dict):
    ...
    for i in range(len(trajectory) - 1, -1, -1):
        connection = trajectory[i]
        key = f"{connection.start_station}-{connection.end_station}"

        if key in needed_connections_dict:
            # keep everything up to and including this connection
            return trajectory[:i + 1]

    # If no connections were found in the dictionary, return an empty list
    return []
...
def create_simulated_annealing_trajectory(coordinates_stations, needed_connections_dict,
    ...
    trajectory.append(current_connection)
    total_duration += current_connection.duration
    current_station = current_connection.end_station
    ...
    while temperature != 0 and len(new_needed_connections_dict) > 0:
        ...
        while not at_destination:
            possible_connections = possible_connections_dict[current_station]

            # check if the current station is already at the start of the nearest connection
            if current_station == nearest_connection.start_station:
                ...
                if total_duration + nearest_connection.duration <= 120:
                    ...
                    new_needed_connections_dict.pop(current_station + '-' + nearest_connection.end_station)
                    current_station = nearest_connection.end_station
                    current_connection = nearest_connection
                    if not trajectory or trajectory[-1].end_station != nearest_connection.end_station:
                        trajectory.append(nearest_connection)
            ...
            connection_key = current_station + '-' + nearest_connection.start_station
            if connection_key in full_connection_dict:
                ...
                if total_duration + full_connection_dict[connection_key].duration <= 120:
                    ...
                    current_station = nearest_connection.start_station
            ...
            for random_station in possible_connections:
                if random_station == current_station:
                    continue

                # ensure connection exists before getting cost
                connection_key = current_station + '-' + random_station
                ...
                current_cost = annealing_cost_function(coordinates_stations,
                    current_connection.start_station, current_station,
                ...
                if total_duration + connection_to_add.duration <= 120:
                    ...
                    trajectory.append(connection_to_add)
                    temperature -= 1
...
def create_dataframe_annealing(trajectories, trajectory_amount,
    ...
    for trajectory in trajectories:
        ...
        for iteration, item in enumerate(trajectory):
