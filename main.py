    ...
    for a in range(iterations):
        ...
        for i in range(trajectory_amount):
            if len(new_needed_connections_dict) > 0:
                ...
                print(f"This is the output of the function: {trajectories_test}")
...
if __name__ == "__main__":
    ...
    full_connection_dict = create_connections(corrected_df)
    original_connection_dict = create_connections(original_df)
    stations = pd.read_csv("data/Noord_Holland/StationsHolland.csv")
    # load data
    test = load_station_objects(stations)
